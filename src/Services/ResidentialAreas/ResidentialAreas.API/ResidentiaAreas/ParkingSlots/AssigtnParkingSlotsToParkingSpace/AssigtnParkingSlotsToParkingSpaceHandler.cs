...
namespace ResidentialAreas.API.ResidentiaAreas.ParkingSlots.AssigtnParkingSlotsToParkingSpace
{
    ...
    public class AssigtnParkingSlotsToParkingSpaceHandler : ICommandHandler<AssigtnParkingSlotsToParkingSpaceCommand, AssigtnParkingSlotsToParkingSpaceResult>
    {
        ...
        private readonly ILogger<AssigtnParkingSlotsToParkingSpaceHandler> _logger;

        public AssigtnParkingSlotsToParkingSpaceHandler(AreaDbContext areaDbContext, IHttpContextAccessor httpContextAccessor, ILogger<AssigtnParkingSlotsToParkingSpaceHandler> logger)
        {
        ...
        public async Task<AssigtnParkingSlotsToParkingSpaceResult> Handle(AssigtnParkingSlotsToParkingSpaceCommand request, CancellationToken cancellationToken)
        {
            ...
            {
                _logger.LogWarning("Assign parking slots to parking space failed: no parking space found with code {ParkingSpaceCode}", request.ParkingSpaceCode);
            ...
            if (!userRoles.Contains("Admin"))
            {
                ...
                {
                    _logger.LogWarning("Assign parking slots to parking space failed: user {UserId} is not authorized for parking space code {ParkingSpaceCode}", userIdClaim.Value, request.ParkingSpaceCode);
                    return new AssigtnParkingSlotsToParkingSpaceResult(null, new ErrorCarrier
                    {
                        Title = "FORBIDDEN",
                        StatusCode = 403,
            ...
            if (slots.Count != slotCodes.Count)
            {
                ...
                _logger.LogWarning("Assign parking slots to parking space failed: missing slot codes {MissingCodes} for parking space code {ParkingSpaceCode}", string.Join(", ", missingCodes), request.ParkingSpaceCode);
                return new AssigtnParkingSlotsToParkingSpaceResult(null, new ErrorCarrier
                {
            ...
            try
            {
                ...
                if (updated == 0)
                {
                    _logger.LogError("Assign parking slots to parking space failed: update returned 0 rows for parking space code {ParkingSpaceCode}", request.ParkingSpaceCode);
                    return new AssigtnParkingSlotsToParkingSpaceResult(null, new ErrorCarrier
                    {
                        Title = "UPDATE_FAILED",
                        StatusCode = 500,
            ...
            {
                _logger.LogError(ex, "Assign parking slots to parking space failed: database error for parking space code {ParkingSpaceCode}", request.ParkingSpaceCode);
                return new AssigtnParkingSlotsToParkingSpaceResult(null, new ErrorCarrier
                {
            ...
            _logger.LogInformation("{Count} parking slot(s) assigned successfully to parking space code {ParkingSpaceCode}", slotCodes.Count, request.ParkingSpaceCode);
