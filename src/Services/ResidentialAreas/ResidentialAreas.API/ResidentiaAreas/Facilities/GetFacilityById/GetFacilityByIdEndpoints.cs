...
namespace ResidentialAreas.API.ResidentiaAreas.Facilities.GetFacilityById
{
    ...
    public class GetFacilityByIdEndpoints : ICarterModule
    {
        ...
        {
            app.MapGet("/residential-areas/facilities/{id:guid}", async (HttpContext httpContext, Guid id, ISender sender, [FromServices] IValidator<Guid> validator, ILogger<GetFacilityByIdEndpoints> logger) =>
            {
                var validationResult = await validator.ValidateAsync(id);
                if (!validationResult.IsValid)
                {
                    logger.LogWarning("Get facility by ID failed: invalid ID {FacilityId}", id);
